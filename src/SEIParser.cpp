...
namespace
{
    ...
    void printout(boost::json::array& elmts, int tag, const char* name, const char* value)
    {
        char t[16];
        sprintf(t, "%#2.2x", tag);
        boost::json::object exif;

        exif["tag"] = t;
        exif["name"] = name;
        exif["value"] = value;

        elmts.push_back(exif);
    }
...
namespace avc2json
{
    ...
    {
        boost::json::object seiobj;
        SEIimpl::RawByteStreamPayload ud;
        sei.payload(std::back_inserter(ud));

        seiobj["type"] = "Picture Timing";
        seiobj["payloadSize"] = (unsigned int)sei.payloadSize();
        boost::json::object payload;
        payload["cpb_removal_delay"] = sei.cpb_removal_delay;
        payload["dpb_output_delay"] = sei.dpb_output_delay;
        payload["pic_struct"] = sei.pic_struct;
        payload["clock_timestamp_flag"] = sei.clock_timestamp_flag;
        payload["ct_type"] = sei.ct_type;
        payload["nuit_field_based_flag"] = sei.nuit_field_based_flag;
        payload["counting_type"] = sei.counting_type;
        payload["full_timestamp_flag"] = sei.full_timestamp_flag;
        payload["discontinuity_flag"] = sei.discontinuity_flag;
        payload["cnt_dropped_flag"] = sei.cnt_dropped_flag;
        payload["n_frames"] = sei.n_frames;
        if (sei.full_timestamp_flag)
        {
            payload["seconds_value"] = sei.seconds_value;
            payload["minutes_value"] = sei.minutes_value;
            payload["hours_value"] = sei.hours_value;
        }
        else
        {
            if (sei.seconds_flag)
        ...
        payload["time_offset"] = sei.time_offset;
        seiobj["payload"] = payload;
        seiobj["timestamp"] = sei.timestamp();
        seiobj["timestamp in seconds"] = sei.timestampInSeconds();
        seipayloads_.push_back(seiobj);
    }
    ...
    void SEIParser::Visit(SEIUserDataUnreg& sei)
    {
        ...
        else
        {
            ...
            seiobj["type"] = "User Data (unregistered)";
            seiobj["payloadSize"] = (unsigned int)sei.payloadSize();
            seiobj["uuid_iso_iec_11578"] = strUUID;
            seiobj["payload"] = payload;
            seipayloads_.push_back(seiobj);
        }
    }
    ...
    void SEIParser::Visit(SEIDefault& sei)
    {
        ...
        seiobj["type"] = sei.type();
        seiobj["payloadSize"] = (unsigned int)sei.payloadSize();
        seiobj["payload"] = strPayload;
        seipayloads_.push_back(seiobj);
    }
    ...
    void SEIParser::parseMDPMData(ThetaStream::SEIUserDataUnreg& sei)
    {
        ...
        seiobj["payloadSize"] = (unsigned)payloadSz;
        strUUID = createUUID(uuid.begin(), uuid.end());
        seiobj["uuid_iso_iec_11578"] = strUUID;

        boost::json::object mdpmObj;
        boost::json::array exifElmts;
