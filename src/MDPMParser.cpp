...
namespace
{
    ...
    void printout(boost::json::array& elmts, unsigned char tag, const char* name, const char* value)
    {
        char t[16];
        sprintf(t, "%#02x", tag);
...
namespace avc2json
{
    MDPMParser::MDPMParser(boost::json::object& nalu)
        :nalu_(nalu)
    {
    }
    ...
    void MDPMParser::Visit(SEIUserDataUnreg& sei)
    {
        ...
        if (ud.size() > 21)
        {
            ...
            else
            {
                ...
                seiObj["user_data_payload_byte"] = val;
            }
        }
